...
def classify_portfolio_impl(positions: Optional[list[dict]] = None,
    ...
    for i, fp in enumerate(financing_positions):
        ...
        rsf_r, asf_r, drag_r = round(rsf_contribution, 1), round(asf_contribution, 1), round(net_nsfr_drag, 1)
        if drag_r < 0:
            nsfr_effect = "improves_stable_funding"
            nsfr_effect_plain = (f"Improves stable funding by ${abs(drag_r):,.1f}mm: the borrow leg credits "
                                 f"${asf_r:,.1f}mm of available stable funding, while the lend leg requires "
                                 f"${rsf_r:,.1f}mm.")
        elif drag_r > 0:
            nsfr_effect = "consumes_stable_funding"
            nsfr_effect_plain = (f"Consumes ${drag_r:,.1f}mm of stable funding: the lend leg requires "
                                 f"${rsf_r:,.1f}mm, while the borrow leg credits only ${asf_r:,.1f}mm of "
                                 f"available stable funding.")
        else:
            nsfr_effect = "neutral"
            nsfr_effect_plain = (f"No net effect on stable funding: the lend leg requires ${rsf_r:,.1f}mm "
                                 f"and the borrow leg credits ${asf_r:,.1f}mm.")
        financing_classified.append({
            ...
            "nsfr_effect": nsfr_effect,
            "nsfr_effect_plain": nsfr_effect_plain,
            "note": "Not counted toward HQLA or RWA (not owned outright). RSF on the lend leg "
                    ...
                    "net_nsfr_drag_mm means this trade consumes stable funding capacity; a NEGATIVE "
                    "value means it adds stable funding, which is an improvement (see nsfr_effect_plain). "
